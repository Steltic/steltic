...
def _restore_zip_into(base, data: bytes):
    ...
    def _allowed(rel: str) -> bool:
        ...
        if "/" not in rel and rel.startswith("report") and rel.endswith(".html"):
            return True
        if rel == "viewer_3d.html":                                   # regenerated by build_report; kept in sync on restore
            return True
...
async def restore_session(request: Request, building: str):
    ...
    # ?archive=1: a job that already holds a design is moved aside first (<building>__<timestamp>, the
    # same convention as a fresh /api/run), so a restore never silently overwrites a design of record.
    # The Nonlinear module's feedback loop uses this when it promotes a verified re-design.
    archived = None
    if str(request.query_params.get("archive", "")).lower() in ("1", "true", "yes") and base.exists() and any(base.iterdir()):
        import datetime
        archived = b + "__" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        try:
            base.rename(JOBS_DIR / archived)
        except Exception as e:
            raise HTTPException(409, f"could not archive the existing job '{b}': {e}")
    try:
        written, had_conv = _restore_zip_into(base, data)
    except ValueError as e:
        raise HTTPException(400, str(e))
    return {"ok": True, "restored": written, "has_conversation": had_conv, "building": b, "archived": archived}
